...
def test_single_image(image_path, model_path, output_path=None, show_result=True):
...
def batch_test(image_dir, model_path, output_dir=None, num_samples=5):
    ...
    for i, img_file in enumerate(test_files):
        print(f"\nProcess Picture{i+1}/{len(test_files)}: {img_file}")
        img_path = os.path.join(image_dir, img_file)
        output_path = os.path.join(output_dir, f'sketch_{img_file}')
        
        test_single_image(img_path, model_path, output_path, show_result=False)
    
    print(f"\n✅ Batch processing complete! Results saved in: {output_dir}")


def visualize_compare(model_path, test_images=None):
...
if __name__ == "__main__":
    ...
    print("="*60)
    test_single_image(TEST_IMAGE, MODEL_PATH, OUTPUT_IMAGE)
